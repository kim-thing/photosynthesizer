...
# SETTINGS
MIDI_ROOT = "large_music"  # 
SEQUENCE_LENGTH = 30
BATCH_SIZE = 32
NUM_EPOCHS = 1 #will change just testing
SAMPLE_SIZE = 100  #  100 random MIDI files
...
def extract_notes_from_midi(midi_path):
    ...
    try:
        ...
        print(f"⚠️ Error reading {midi_path}: {e}") #found an error symbol that works
    return notes
...
if len(all_midi_files) < SAMPLE_SIZE:
    raise ValueError(f"Not enough MIDI files, there's only {len(all_midi_files)} heree.")
sampled_files = random.sample(all_midi_files, SAMPLE_SIZE)

#Step 3 create note dataset
all_notes = []
